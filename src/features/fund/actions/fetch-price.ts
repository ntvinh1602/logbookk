...
export const fetchPrices = createServerFn({ method: 'POST' }).handler(
  async (): Promise<PriceRefreshResult> => {
    ...
    if (rpcError) {
      throw new Error(
        rpcError.message ?? 'Failed to fetch active stock tickers',
      )
    }
    ...
    const failedTickers = tickers.filter(
      (ticker) => !assetIdByTicker.has(ticker),
    )
    const mappedTickers = tickers.filter((ticker) =>
      assetIdByTicker.has(ticker),
    )
