...
CREATE TABLE IF NOT EXISTS "dim"."assets" (
...
ALTER TABLE "dim"."assets" OWNER TO "postgres";


ALTER TABLE "dim"."assets" ALTER COLUMN "id" ADD GENERATED BY DEFAULT AS IDENTITY (
    SEQUENCE NAME "dim"."assets_id_seq"
    START WITH 1
    INCREMENT BY 1
    NO MINVALUE
    NO MAXVALUE
...
CREATE TABLE IF NOT EXISTS "dim"."currencies" (
    "code" "text" NOT NULL,
    "name" "text" NOT NULL
);


ALTER TABLE "dim"."currencies" OWNER TO "postgres";
...
COMMENT ON TABLE "dwd"."tx_entries" IS 'Transaction events';
...
CREATE OR REPLACE VIEW "flight"."routes_geojson" WITH ("security_invoker"='on') AS
 ...
 SELECT "gen_random_uuid"() AS "id",
    ...
    "round"(((((6371.0 * (2)::numeric))::double precision * "asin"("sqrt"(("power"("sin"(("radians"(("b"."lat" - "a"."lat")) / (2)::double precision)), (2)::double precision) + (("cos"("radians"("a"."lat")) * "cos"("radians"("b"."lat"))) * "power"("sin"(("radians"(("b"."lng" - "a"."lng")) / (2)::double precision)), (2)::double precision)))))))::numeric, 1) AS "distance_km",
...
ALTER TABLE ONLY "dim"."currencies"
...
ALTER TABLE ONLY "dim"."assets"
    ADD CONSTRAINT "assets_currency_code_fkey" FOREIGN KEY ("currency_code") REFERENCES "dim"."currencies"("code") ON UPDATE CASCADE;
...
ALTER TABLE "dim"."assets" ENABLE ROW LEVEL SECURITY;
